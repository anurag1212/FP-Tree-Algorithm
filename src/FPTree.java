...
public class FPTree {
    ...
    private double max_pairwise_lift;
    private HashSet<String> max_pair;
    private Hashtable<String, Double> Flist = new Hashtable<String, Double>();
    private ArrayList<String> FArray = new ArrayList<String>();
    private static long treeSize = 1;
	...
	{		
		...
		Hashtable<HashSet<String>, Integer> intersectionFreq = new Hashtable<HashSet<String>, Integer>();

		try 
		{
			firstScan(inputfile, items_frequency,intersectionFreq); //fp-tree header table will be created in the first scan
			System.out.println(intersectionFreq);
		} 
		...
		try 
		{
			secondScan(inputfile, FArray); //fp-tree will be created in the second scan
		} 
		catch(IOException ioe) 
		{
	...
	{		
		...
		try 
		{
			firstScan(inputfile, items_frequency, start_at, end_at,intersectionFreq); //fp-tree header table will be created in the first scan
		} 
		catch(IOException ioe) 
		{
			System.out.println("Error! First scan for FPTree construction failed !");
	...
	{		
		Hashtable<String, Integer> items_frequency = new Hashtable<String, Integer>();
		Hashtable<HashSet<String>, Integer> intersectionFreq = new Hashtable<HashSet<String>, Integer>();
		firstScan(cond_pattern_base, items_frequency,intersectionFreq); 	//fp-tree header table will be created in the first scan
	 ...
	 */
	//WORKING ON THIS
	public void extractItems(String transaction, int count, Hashtable<String,Integer> items_frequency, Hashtable<HashSet<String>,Integer> intersectionFreq)
	{
		int lastfreq=1;
    	...
    	{ 
			...
			for(int j=i+1; j<items.length; j++)
			{
				...
				temp_set.add(items[j]);
				if(intersectionFreq.containsKey(temp_set))
					intfreq = intersectionFreq.get(temp_set)+count;
				intersectionFreq.put(temp_set, new Integer(intfreq));
			}
    	}	
		if(items_frequency.containsKey(items[items.length-1])) 			//if this item already encountered before
			lastfreq = items_frequency.get(items[items.length-1]) + count; //increment frequency
		items_frequency.put(items[items.length-1],new Integer(lastfreq)); //update item with new frequency
	}
	 ...
	 */
	public void insertIntoFPTree(String prefix, int count, ArrayList<String> FArray)
	{
        ...
        for(int i=0;i<FArray.size();i++){
            if(itemList.contains(FArray.get(i))){
                int frequency = Flist.get(FArray.get(i)).intValue();
                if(frequency>=support_threshold)
                    aie.add(new ItemElement(FArray.get(i),frequency));
            }
        }
...
//    	Collections.sort(aie); 	//prefix is sorted, we will now add it to the FPTree 
	...
	*/	
	//WORKING ON THIS
	public void firstScan(File inputfile, Hashtable<String,Integer> items_frequency,Hashtable<HashSet<String>,Integer> intersectionFreq) throws IOException
	{
		BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(inputfile)));
        String transaction = null;
        int transactionCount = 0;
        //double max_pairwise_lift = 0.0;
        //HashSet<String> max_pair = new HashSet<String>(); 
        double lift;
        while((transaction = br.readLine()) != null) //read input file (transaction database)
        {
        	extractItems(transaction, 1, items_frequency,intersectionFreq);
        	transactionCount++;
        }
        br.close(); //file reading complete
        Hashtable<HashSet<String>,Double> pairwise_lift = new Hashtable<HashSet<String>,Double>();
        Iterator i = intersectionFreq.entrySet().iterator();
        while(i.hasNext())
        {
        	Map.Entry pair = (Map.Entry) i.next();
        	HashSet<String> temp = new HashSet<String>();
        	temp = (HashSet<String>)pair.getKey();
        	int temp_count = Integer.parseInt(pair.getValue().toString());
        	double numerator=0.0;
        	double denominator=1.0;
        	for(String s : temp)
        	{
        		if(items_frequency.containsKey(new String(s))&&items_frequency.get(s)>=support_threshold)
        		{
        			numerator+=items_frequency.get(s);
        			denominator*=items_frequency.get(s);
        		}
        		else
        		{
        			denominator=0;
        	...
        	int sup1 = 0;
        	int sup2 = 0;
        	numerator-=intersectionFreq.get(temp);
        	if(denominator>0.0)
        	{
            	lift = numerator/denominator*transactionCount;
            	if(lift > max_pairwise_lift){
            		max_pairwise_lift = lift;
            		max_pair = temp;
            	}
            	else if(lift == max_pairwise_lift){
            		//select one with greater support
                    
                    for(String s : max_pair){
                        sup1 += items_frequency.get(s);
                    }
                    for(String s : temp){
                        sup2 += items_frequency.get(s);
                    }
                    if(sup2 > sup1){
                          max_pairwise_lift = lift;
            		      max_pair = temp;
            	...
            	pairwise_lift.put(temp, lift);
        	}
        }
        //System.out.println("Intersection freq" + intersectionFreq);
        //System.out.println(pairwise_lift);
        createFPTreeHeaderTable(items_frequency);
        //System.out.println(max_pairwise_lift + " " + max_pair);
        
        //calculating Flist
        double max_value = 0;
        String item = "";
        for(String s : max_pair){
            if(items_frequency.get(s) > max_value){
                max_value = items_frequency.get(s);
                item = s;
            }
        }
        FArray.add(item);
        Flist.put(item,max_value);
        
        for(String s : max_pair){
            if(!Flist.containsKey(s)){
                Flist.put(s, (double)items_frequency.get(s));
                FArray.add(s);
            }
        }
        
        Hashtable<String, Double> freqList = new Hashtable<String, Double>();
        Set<String> keys = items_frequency.keySet();
        for(String key : keys){
            if(!Flist.containsKey(key))
                freqList.put(key, 0.0);
        }
        
        String max_index = "";
        double max_fvalue = 0.0;
        keys = freqList.keySet();
        for(String key : keys){
            HashSet<String> temp = new HashSet<String>();
            temp.add(key);
            Set<String> FlistKeys = Flist.keySet();
            double sum = 0.0;
            for(String fkey : FlistKeys){
                temp.add(fkey);
                if(pairwise_lift.containsKey(temp))
                    sum += pairwise_lift.get(temp);
                temp.remove(fkey);
            }
            freqList.put(key,sum);
            if(sum > max_fvalue){
                max_fvalue = sum;
                max_index = key;
            }
        }
        //System.out.println("initial freq list: " + freqList);
        Flist.put(max_index, (double)items_frequency.get(max_index));
        FArray.add(max_index);
        freqList.remove(max_index);
        String latestEl = max_index;
        ...
        while(!freqList.isEmpty()){
            ...
            //System.out.println("freq list :" + freqList);
            Flist.put(max_index, (double)items_frequency.get(max_index));
            FArray.add(max_index);
            freqList.remove(max_index);
            latestEl = max_index;
        ...
        //System.out.println("F list :" + FArray);
	}

	//overloaded method - file reading done between lines [start_at] and [end_at]
	public void firstScan(File inputfile, Hashtable<String,Integer> items_frequency, int start_at, int end_at,
			Hashtable<HashSet<String>,Integer> intersectionFreq) throws IOException
	{       
		BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(inputfile)));
        String transaction = null;
        int line_no = 0;
	 ...
	 */
	public void secondScan(File inputfile, ArrayList<String> FArray) throws IOException
	{       
		BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(inputfile)));
        String transaction = null;
        
        while((transaction = br.readLine()) != null) //read input file (transaction DB) again
        	insertIntoFPTree(transaction, 1, FArray);
	 ...
	 * each prefix is in the format <support>:<space separated item list>
		*/
	public void firstScan(ArrayList<String> cond_pattrn_base, Hashtable<String,Integer> items_frequency,Hashtable<HashSet<String>,Integer> intersectionFreq)
	{       
		for(int n=0; n<cond_pattrn_base.size(); n++)
		{
			...
			extractItems(pattern, count, items_frequency,intersectionFreq);			
...
//			insertIntoFPTree(pattern, count, items_frequency);	//prefix inserted into FPTree
	...
	{
		//for(int i=0; i<depth; i++)	//tabs printed as per the depth		//PRINTCOMMENT
			//System.out.print("\t");		//PRINTCOMMENT
		//System.out.print("-" + node + "\n");	//print node : its support : no. of children		//PRINTCOMMENT
	...
	public void traverseFPTreeHeaderTable()
	{
		for(int i=0; i<header_table.size(); i++)
		{
			//System.out.print(header_table.get(i).getItem()+": "); //print item		//PRINTCOMMENT
			FPTreeNode tmp = header_table.get(i).getNodeLink();
			while(tmp!=null)
			{
				//System.out.print("-> "+tmp);		//PRINTCOMMENT
				tmp = tmp.getNextNode();
			}
			//System.out.print("\n");		//PRINTCOMMENT
		}	
	...
	public void genAllCombinations(FPTreeNode fptn, String curr)
	{
		...
		//System.out.print(curr + " : " + fptn.getFrequency() + "\n");	//print combination with support		//PRINTCOMMENT
	...
	public void minePatternsByFPGrowth(String curr)
	{
		...
		else
		{
			for(int i=header_table.size()-1; i>=0; i--)
			{
				...
				//System.out.print(itemset + "\n");	//print frequent item set containing this element		//PRINTCOMMENT
    ...
    public long computeTreeMemory() throws Exception
	{
		long tree_size = 0;		
		for(int i=header_table.size()-1; i>=0; i--)
		{
	...
	public static void main(String args[]) throws Exception
	{
		...
		int min_supp = 5000;
		//int min_supp = Integer.parseInt(args[1]);
/*
		if(args.length>2)
		{
...
*/
		input_data_filename = "src/retail.dat";// + args[0];
        //input_data_filename = args[0];
		File file = new File(input_data_filename);
		
		long startTime, endTime, totalTime,buildTime,mineTime,numNodes,treeMemory;  

		startTime = System.currentTimeMillis();
		FPTree fpt = new FPTree(file, min_supp);
		endTime   = System.currentTimeMillis();
		buildTime = totalTime = endTime - startTime;

//		System.out.println("\n");
//		System.out.println("TreeSize: " + fpt.computeTreeSize() + "\t " + "BuildingTime: " + totalTime);
//        System.out.println("Total Tree Memory " + fpt.computeTreeMemory());
//		System.out.println("\n");
//		fpt.printTreeDetails();
//		System.out.println("\n");
        //numNodes = fpt.computeTreeSize();
        treeMemory = fpt.computeTreeMemory();
		...
		endTime   = System.currentTimeMillis();
		mineTime = totalTime = endTime - startTime;
		
        //System.out.println("Mining Time : " + mineTime);
        totalTime = buildTime + mineTime;
        ...
        System.out.println("Mine time: "+mineTime );
        System.out.println("Total time: "+totalTime );
        System.out.println("Number of nodes" + treeSize);
        fpt.printTreeDetails();
        System.out.println("\n");
        System.out.println("Tree Memory : " + treeMemory);
